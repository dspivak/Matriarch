...
def rotateAxis(axis, angle, fixedpoint=[0, 0, 0]):
    def rotate(point, axis, angle):
        ...
        M = [
            [
                a * a
                + (-c * v + b * w)
                * (math.sin(angle) * u + math.cos(angle) * (-c * v + b * w))
                + u * (math.cos(angle) * u - math.sin(angle) * (-c * v + b * w)),
                a * b
                + (c * u - a * w)
                * (math.sin(angle) * u + math.cos(angle) * (-c * v + b * w))
                + v * (math.cos(angle) * u - math.sin(angle) * (-c * v + b * w)),
                a * c
                + (-b * u + a * v)
                * (math.sin(angle) * u + math.cos(angle) * (-c * v + b * w))
                + w * (math.cos(angle) * u - math.sin(angle) * (-c * v + b * w)),
            ],
            [
                a * b
                + (-c * v + b * w)
                * (math.sin(angle) * v + math.cos(angle) * (c * u - a * w))
                + u * (math.cos(angle) * v - math.sin(angle) * (c * u - a * w)),
                b * b
                + (c * u - a * w)
                * (math.sin(angle) * v + math.cos(angle) * (c * u - a * w))
                + v * (math.cos(angle) * v - math.sin(angle) * (c * u - a * w)),
                b * c
                + (-b * u + a * v)
                * (math.sin(angle) * v + math.cos(angle) * (c * u - a * w))
                + w * (math.cos(angle) * v - math.sin(angle) * (c * u - a * w)),
            ],
            [
                a * c
                + u * (-math.sin(angle) * (-b * u + a * v) + math.cos(angle) * w)
                + (-c * v + b * w)
                * (math.cos(angle) * (-b * u + a * v) + math.sin(angle) * w),
                b * c
                + v * (-math.sin(angle) * (-b * u + a * v) + math.cos(angle) * w)
                + (c * u - a * w)
                * (math.cos(angle) * (-b * u + a * v) + math.sin(angle) * w),
                c * c
                + w * (-math.sin(angle) * (-b * u + a * v) + math.cos(angle) * w)
                + (-b * u + a * v)
                * (math.cos(angle) * (-b * u + a * v) + math.sin(angle) * w),
            ],
        ]
    ...
    return lambda x: translate(fixedpoint)(
        rotate(translate(minus(fixedpoint))(x), axis, angle)
    )
...
def changefixedpoint(transformation, fixedpoint=[0, 0, 0]):
    return lambda x: translate(fixedpoint)(
        transformation(translate(minus(fixedpoint))(x))
    )
...
def orthogonalComponent(vec, thingToWhichOutputIsOrthogonal):
    ...
    lengthSquared = norm(thingToWhichOutputIsOrthogonal) * norm(
        thingToWhichOutputIsOrthogonal
    )
    return translate(
        scale(thingToWhichOutputIsOrthogonal, -innerProductValue / lengthSquared)
    )(vec)
...
def Nlocation(newC, newCA, newO):
    ...
    transformation = orthogonal(
        CtoCA, crossproduct(CtoCA, CtoO), newCtoCA, crossproduct(newCtoCA, newCtoO)
    )
...
def Tlist(list1, list2):
    return [
        T(list1[0], list1[1], list1[2], list2[0], list2[1], list2[2]),
        orthogonal(list1[1], list1[2], list2[1], list2[2]),
    ]
