...
def stringToatom(string):
    str1, number, str2, resnumber, str3, x, y, z, str4, pepnumber, str5 = (
        string[0:6],
        string[6:11],
        string[11:22],
        string[22:26],
...
class block(list):
    ...
    def __str__(self, minatom=1, resnumber=1, pepnumber=1):
        ...
        for i in range(0, len(self)):
            output += self[i].__str__(
                temp[0] + minatom, temp[1] + resnumber, temp[2] + pepnumber
            )
            for j in range(0, 3):
        ...
        if type(self) == chain:
            output += (
                "TER   "
                + floatTostring(temp[0] + minatom, 5, 0)
                + space(6)
                + self[-1][-1].str2[6:]
                + floatTostring(resnumber + temp[1] - 1, 4, 0)
            )
...
class amino(block):
    ...
    def boundarycompute(self):
        try:
            ...
            atoms.remove(0)
            posterm = vector.Nlocation(
                atoms[1].coords(), atoms[2].coords(), atoms[3].coords()
            )
            axis = vector.translate(vector.minus(atoms[0].coords()))(posterm)
            N1 = vector.unit(axis)
            length = vector.norm(axis)
            N2 = vector.unit(
                vector.translate(vector.minus(atoms[2].coords()))(atoms[4].coords())
            )
